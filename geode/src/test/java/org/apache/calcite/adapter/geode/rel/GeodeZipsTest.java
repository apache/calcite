...
public class GeodeZipsTest extends AbstractGeodeTest {
  ...
  @Test
  public void testWhereWithOrNumeric() {
    calciteAssert()
            .query("SELECT state, city FROM view WHERE pop = 65046 OR pop = 57871")
            .returnsCount(2)
            .queryContains(
                    GeodeAssertions.query("SELECT state AS state, city AS city FROM /zips " +
                            "WHERE pop IN SET (65046, 57871)"));
  }

  @Test
  public void testWhereWithOrString() {
    calciteAssert()
            .query("SELECT state, city FROM view WHERE city = 'MANCHESTER' OR city = 'NASHUA'")
            .returnsCount(2)
            .queryContains(
                    GeodeAssertions.query("SELECT state AS state, city AS city FROM /zips " +
                            "WHERE city IN SET ('MANCHESTER', 'NASHUA')"));
  }
}
