...
public class GeodeBookstoreTest extends AbstractGeodeTest {
  ...
  public void testWhereWithAndOr() {
    calciteAssert()
            ...
            .explainContains("PLAN=GeodeToEnumerableConverter\n"
                    ...
                    + "      GeodeTableScan(table=[[geode, BookMaster]])\n"
                    + "\n")
            .queryContains(GeodeAssertions.query("SELECT author AS author FROM /BookMaster WHERE "
                    + "(itemNumber > 123 AND itemNumber = 789) OR author = 'Daisy Mae West'"));
  }
  ...
  @Test
  public void testFilterWithNestedFieldOrPredicate() {
    calciteAssert()
            .query("SELECT primaryAddress['city'] AS city\n"
                    + "FROM geode.BookCustomer\n"
                    + "WHERE primaryAddress['city'] = 'Topeka' OR primaryAddress['city'] = 'San Francisco' \n")
            .returnsCount(3)
            .queryContains(GeodeAssertions.query("SELECT primaryAddress.city AS city FROM /BookCustomer WHERE " +
                    "primaryAddress.city = 'Topeka' OR primaryAddress.city = 'San Francisco'"));
  }

  @Test
  public void testSqlBookMasterWhereMultipleBooleanValues() throws SQLException {
    calciteAssert().query("select author, title from geode.BookMaster "
            + "WHERE testBoolean = true OR testBoolean = false")
            .returnsCount(2)
            .queryContains(GeodeAssertions.query("SELECT author AS author, title AS title FROM /BookMaster WHERE " +
                    "testBoolean IN SET (true, false)"));
  }
}
