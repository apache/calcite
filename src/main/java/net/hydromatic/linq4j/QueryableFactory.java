...
public interface QueryableFactory<T> {
    ...
    <TAccumulate> TAccumulate aggregate(
        Queryable<T> source,
        TAccumulate seed,
        FunctionExpression<Function2<TAccumulate, T, TAccumulate>>
            selector);
     ...
     * specified type. */
    <TResult> Queryable<TResult> ofType(
        Queryable<T> source,
        Class<TResult> clazz);

    <T2> Queryable<T2> cast(
        Queryable<T> source,
        Class<T2> clazz);

    /** Sorts the elements of a sequence in ascending
     * order according to a key. */
    <TKey extends Comparable> OrderedQueryable<T> orderBy(
        Queryable<T> source,
        FunctionExpression<Function1<T, TKey>> keySelector);
