...
public class ParquetColdStartPerformanceTest {
  ...
  private void runColdStartTest(String testName, String queryTemplate) throws Exception {
    ...
    results.put("CSV_LINQ4J", testWithEngine("linq4j", "sales.csv", queryTemplate, false));

    // Test 2: CSV with PARQUET engine (will use cached .parquet_cache)
    results.put("CSV_PARQUET_CACHED", testWithEngine("parquet", "sales.csv", queryTemplate, true));

    // Test 3: CSV with ARROW engine
    results.put("CSV_ARROW", testWithEngine("arrow", "sales.csv", queryTemplate, false));

    // Test 4: CSV with VECTORIZED engine
    results.put("CSV_VECTORIZED", testWithEngine("vectorized", "sales.csv", queryTemplate, false));

    displayResults(results);
  }

  private TestResult testWithEngine(String engine, String fileName, String queryTemplate, boolean expectCached) {
  ...
  private void createNativeParquetFile() throws Exception {
    ...
    try (Connection connection = DriverManager.getConnection("jdbc:calcite:");
      ...
      // This simulates having a native Parquet file that wasn't created by auto-conversion
  ...
  private void printInsights() {
    ...
    System.out.println("   - Each test run creates a new connection (simulating restart)");
    System.out.println("   - No warmup runs to test true cold performance");
    System.out.println("   - CSV_PARQUET_CACHED uses pre-converted files from .parquet_cache");

    System.out.println("\n2. Expected Results:");
    System.out.println("   - CSV_LINQ4J: Baseline (parsing CSV on every query)");
    System.out.println("   - CSV_PARQUET_CACHED: Should show 10-30x speedup (no conversion needed)");
    System.out.println("   - PARQUET_* variants: Direct Parquet file access");

    System.out.println("\n3. Key Findings:");
    System.out.println("   - Pre-converted Parquet files eliminate conversion overhead");
    System.out.println("   - Columnar format benefits are more apparent without conversion cost");
    System.out.println("   - Restart scenarios show true Parquet performance advantage");
  }
