...
public class LargeScaleEnginePerformanceTest {
    ...
    for (int size : sizes) {
      ...
      for (String engine : engines) {
        try {
          ...
          System.out.println("  " + engine.toUpperCase(Locale.ROOT)
              + ": FAILED - " + e.getMessage());
        }
      }
  ...
  private long measureQueryPerformance(String engine, String fileName,
  ...
  private int runQuery(String engine, String fileName, String queryTemplate) throws Exception {
    try (Connection connection = DriverManager.getConnection("jdbc:calcite:");
      ...
      SchemaPlus fileSchema =
          rootSchema.add(
              "PERF_" + engine.toUpperCase(Locale.ROOT), FileSchemaFactory.INSTANCE.create(rootSchema,
              "PERF_" + engine.toUpperCase(Locale.ROOT), operand));
  ...
  private void createSalesDataset(int rows) throws IOException {
    ...
    System.out.println("  Creating " + file.getName() + " with "
        + String.format(Locale.ROOT, "%,d", rows) + " rows...");
