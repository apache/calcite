...
public class MultiTableExcelTest {
  ...
  @Test public void testSingleTableMode() throws Exception {
    // Test that without multiTableExcel flag, we get single table behavior
    try (Connection connection = DriverManager.getConnection("jdbc:calcite:");
         CalciteConnection calciteConnection = connection.unwrap(CalciteConnection.class)) {
      ...
      try (Statement statement = connection.createStatement()) {
        // Should find single table with all data
        ResultSet tables = connection.getMetaData().getTables(null, "excel", "%", null);
        int tableCount = 0;
        while (tables.next()) {
          String tableName = tables.getString("TABLE_NAME");
          if (tableName.startsWith("multi_table_test")) {
            tableCount++;
          }
        }
        // In single table mode, should only find one table
