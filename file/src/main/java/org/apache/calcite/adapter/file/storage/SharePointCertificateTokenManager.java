...
public class SharePointCertificateTokenManager extends SharePointRestTokenManager {
  ...
  @Override protected void refreshWithClientCredentials() throws IOException {
    try {
      ...
      // Build request body
      String requestBody =
          String.format("grant_type=client_credentials&client_id=%s&scope=%s&" +
  ...
  private String createClientAssertion(String audience) throws Exception {
    // JWT Header
    String header =
        String.format("{\"alg\":\"RS256\",\"typ\":\"JWT\",\"x5t\":\"%s\"}",
        thumbprint);

    // JWT Payload
    long now = System.currentTimeMillis() / 1000;
    String payload =
        String.format("{\"aud\":\"%s\",\"exp\":%d,\"iss\":\"%s\",\"jti\":\"%s\",\"nbf\":%d,\"sub\":\"%s\",\"iat\":%d}",
        audience,
