...
/**
 * JDBC Driver that automatically recovers materialized views on startup.
 *
 * <p>This driver extends the standard Calcite JDBC driver to provide:
 * <ul>
 *   <li>Automatic schema registration on connection</li>
 *   <li>Materialized view discovery and recovery</li>
 *   <li>Transparent restart survival</li>
 *   <li>Configuration-driven setup</li>
 * </ul>
 *
 * <p>Connection URL format:
 * {@code jdbc:calcite:schema=file;storage_path=/mnt/ssd/mv_storage;engine=parquet}
 */
public class MaterializedViewJdbcDriver extends Driver {

  private static final Logger LOGGER = Logger.getLogger(MaterializedViewJdbcDriver.class.getName());
  ...
  @Override public Connection connect(String url, Properties info) throws SQLException {
    ...
    LOGGER.info("Creating MaterializedView-enabled Calcite connection: " + url);

    // Get underlying Calcite connection
    Connection connection = super.connect(url, info);
    CalciteConnection calciteConnection = connection.unwrap(CalciteConnection.class);

    // Automatically setup schemas and recover materialized views
  ...
  /**
   * Automatically setup schemas and recover materialized views based on connection configuration.
   */
  private void setupSchemasAndRecoverMaterializedViews(CalciteConnection connection,
    ...
    try {
      ...
      LOGGER.severe("Failed to setup schemas and recover materialized views: " + e.getMessage());
      throw new RuntimeException("Schema setup failed", e);
    }
  }

  /**
   * Setup a single schema with materialized view recovery.
   */
  private void setupSchema(SchemaPlus rootSchema, SchemaConfig schemaConfig) {
    ...
    try {
      LOGGER.info("Setting up schema: " + schemaConfig.getName());
      ...
      // Create execution engine config with materialized view storage
  ...
  private MaterializedViewConfig parseConfiguration(String url, Properties info) {
    ...
    // Format: jdbc:calcite:schema=file;storage_path=/mnt/ssd;engine=parquet
    ...
    if (storagePath == null) {
      storagePath = System.getProperty("java.io.tmpdir") + "/calcite_materialized_views";
  ...
  private void addConfiguredSchemas(MaterializedViewConfig config, Properties info) {
    ...
    // schema.sales.storage_path=/mnt/ssd/sales_mv
  ...
  @Override protected DriverVersion createDriverVersion() {
    return new DriverVersion(
        "Calcite Materialized View JDBC Driver",
        "1.0.0",
        "Apache Calcite",
        "1.41.0-SNAPSHOT",
        true,
  ...
  /**
   * Configuration holder for materialized view setup.
   */
  private static class MaterializedViewConfig {
  ...
  /**
   * Configuration for a single schema.
   */
