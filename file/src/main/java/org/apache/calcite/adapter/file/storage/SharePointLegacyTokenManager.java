...
public class SharePointLegacyTokenManager extends SharePointTokenManager {
  ...
  protected void refreshWithClientCredentials() throws IOException {
    ...
    String sharePointResource = String.format("%s@%s", 
        "00000003-0000-0ff1-ce00-000000000000", // SharePoint principal ID
        effectiveRealm);
  ...
  private String discoverRealm() throws IOException {
    URI uri = URI.create(getSiteUrl());
    URL url = URI.create(String.format("https://%s/_vti_bin/client.svc", 
        uri.getHost())).toURL();
