...
/**
 * ResultSet wrapper implementation that provides timezone-independent TIME handling.
 * Intercepts getTime() calls and compensates for timezone conversion.
 */
public class FileResultSetWrapperImpl implements ResultSet {
  private static final Logger LOGGER = LoggerFactory.getLogger(FileResultSetWrapperImpl.class);

  private final ResultSet delegate;

  public FileResultSetWrapperImpl(ResultSet delegate) {
  ...
  @Override public Time getTime(int columnIndex) throws SQLException {
    ...
    Time compensated = FileResultSetWrapper.compensateTimeForTimezone(originalTime);
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug("TIME compensation applied: original={}, compensated={}",
          originalTime.getTime(), compensated.getTime());
    }
    return compensated;
  }
  ...
  @Override public Time getTime(String columnLabel) throws SQLException {
    ...
    return FileResultSetWrapper.compensateTimeForTimezone(originalTime);
  }

  @Override public Time getTime(int columnIndex, Calendar cal) throws SQLException {
    // For TIME with Calendar, still apply compensation
    Time originalTime = delegate.getTime(columnIndex, cal);
    if (originalTime == null) {
      return null;
    }
    return FileResultSetWrapper.compensateTimeForTimezone(originalTime);
  ...
  @Override public Time getTime(String columnLabel, Calendar cal) throws SQLException {
    // For TIME with Calendar, still apply compensation
    Time originalTime = delegate.getTime(columnLabel, cal);
    if (originalTime == null) {
      return null;
    }
    return FileResultSetWrapper.compensateTimeForTimezone(originalTime);
  ...
  @Override public Object getObject(int columnIndex) throws SQLException {
    ...
    // Check if this is a TIME column by looking at metadata
    ResultSetMetaData metaData = delegate.getMetaData();
    if (metaData.getColumnType(columnIndex) == java.sql.Types.TIME) {
      // If it's an Integer (milliseconds since midnight), preserve it
      if (obj instanceof Integer) {
        // Return the raw integer value for TIME columns
        return obj;
      }
      // If it's already a Time object, we need to compensate
      if (obj instanceof Time) {
        return FileResultSetWrapper.compensateTimeForTimezone((Time) obj);
      }
    }
  ...
  @Override public Object getObject(String columnLabel) throws SQLException {
    ...
    // Check if this is a TIME column by looking at metadata
    int columnIndex = delegate.findColumn(columnLabel);
    ResultSetMetaData metaData = delegate.getMetaData();
    if (metaData.getColumnType(columnIndex) == java.sql.Types.TIME) {
      // If it's an Integer (milliseconds since midnight), preserve it
      if (obj instanceof Integer) {
