...
public class DruidAdapterIT {
  ...
  @Test public void testFieldBasedCostColumnPruning() {
    ...
    String druidQuery = "{'queryType':'scan',"
        + "'dataSource':'wikiticker',"
        + "'intervals':['1900-01-09T00:00:00.000Z/2992-01-10T00:00:00.000Z'],"
        + "'virtualColumns':[{'type':'expression','name':'vc','expression':'\\'__time\\'',"
        + "'outputType':'LONG'}],'columns':['vc','countryName'],"
        + "'resultFormat':'compactedList'";
  ...
  @Test public void testPushComplexFilter() {
    ...
    String druidQuery = "{'queryType':'timeseries','dataSource':'foodmart',"
        ...
        + "{'type':'bound','dimension':'__time','lower':'1997','lowerStrict':false,'upper':'1997',"
    ...
    sql(sql)
        ...
        .queryContains(
            druidChecker("{'queryType':'groupBy','dataSource':'foodmart',"
                ...
                + "'timeZone':'UTC','locale':'en-US'}},{'type':'bound','dimension':'__time',"
                + "'lower':'11','lowerStrict':false,'upper':'11','upperStrict':false,"
                + "'ordering':'numeric','extractionFn':{'type':'timeFormat',"
                + "'format':'M','timeZone':'UTC','locale':'en-US'}},{'type':'bound',"
                + "'dimension':'__time','lower':'1997','lowerStrict':false,'upper':'1997',"
  ...
  @Test public void testExtractFilterWorkWithPostAggregations() {
    ...
    final String druidQuery = "'filter':{'type':'and','fields':[{'type':'selector','dimension'"
        + ":'brand_name','value':'Bird Call'},{'type':'or','fields':[{'type':'bound',"
        + "'dimension':'__time','lower':'10','lowerStrict':false,'upper':'10','upperStrict':false,"
        + "'ordering':'numeric','extractionFn':{'type':'timeFormat','format'"
        + ":'w','timeZone':'UTC','locale':'en-US'}},{'type':'bound','dimension':'__time',"
        + "'lower':'11','lowerStrict':false,'upper':'11','upperStrict':false,'ordering':'numeric',"
        + "'extractionFn':{'type':'timeFormat','format':'w','timeZone':'UTC'"
  ...
  @Test public void testPartiallyPostAggregation() {
    ...
    final String plan = "PLAN=EnumerableInterpreter\n"
        ...
        + " aggs=[[SUM($2), SUM($3), SUM($1)]], post_projects=[[$0, /($1, $2), $3]],"
        + " sort0=[1], dir0=[DESC])";
    sql(sqlQuery, FOODMART)
    ...
    sql(sql, WIKI)
        ...
        .queryContains(
            druidChecker("{'queryType':'scan','dataSource':'wikiticker',"
                ...
                + "'\\'__time\\'','outputType':'LONG'}],"
                + "'columns':['vc','channel','cityName','comment','countryIsoCode','countryName',"
  ...
  @Test public void testFilterWithFloorOnTime() {
    ...
    final String druidQueryPart4 = "\"columns\":[\"vc\"]";
  ...
  @Test public void testTimeWithFilterOnFloorOnTimeAndCastToTimestamp() {
    ...
    final String druidQueryPart2 = "'granularity':'month','timeZone':'UTC',"
        + "'locale':'en-US'}},'virtualColumns':[{'type':'expression',"
        + "'name':'vc','expression':'timestamp_floor(\\'__time\\'";
  ...
  // "SELECT COUNT(*) FROM foo GROUP BY CAST((EXTRACT(MONTH FROM __time) - 1 ) / 3 + 1 AS INTEGER)"
        ...
        .queryContains(
            druidChecker(
                "{\"queryType\":\"groupBy\",\"dataSource\":\"foodmart\","
                    + "\"granularity\":\"all\",\"dimensions\":[{\"type\":\"default\","
                    + "\"dimension\":\"city\"}],\"limitSpec\":{\"type\":\"default\"},"
                    + "\"aggregations\":[{\"type\":\"count\",\"name\":\"EXPR$0\"}],"
                    + "\"intervals\":[\"1900-01-09T00:00:00.000Z/2992-01-10T00:00:00.000Z\"]}"))
        .returnsUnordered("EXPR$0=86829");
  }
  ...
  public void testExpressionsFilterWithCastTimeToDateToChar2() {
    ...
    sql(sql, FOODMART)
        .queryContains(druidChecker(druidQuery))
        .returnsUnordered("C=117; EXPR$1=1997-01-01 00:00:00");
  }
  ...
  public void testExtractHourFilterExpression() {
    ...
    CalciteAssert.that()
        ...
        .queryContains(druidChecker(filterPart1, "America/Los_Angeles", "HOUR"));
    ...
    CalciteAssert.that()
        ...
        .queryContains(druidChecker(filterPart1, "EST", "HOUR"));
    ...
    CalciteAssert.that()
        ...
        .queryContains(druidChecker(filterPart1, "UTC", "HOUR"));
  }
  ...
  public void testTimeFloorExpressions() {
    ...
    final String plan = "PLAN=EnumerableInterpreter\n"
        + "  BindableProject(D=[CAST($0):TIMESTAMP(0) NOT NULL])\n"
        + "    BindableSort(sort0=[$0], dir0=[ASC], fetch=[3])\n"
        + "      DruidQuery(table=[[foodmart, foodmart]], "
        + "intervals=[[1900-01-09T00:00:00.000Z/2992-01-10T00:00:00.000Z]], "
        + "filter=[=(CAST(FLOOR(CAST($0):DATE NOT NULL, FLAG(MONTH))):DATE NOT NULL, 1997-01-01)], "
        + "projects=[[FLOOR($0, FLAG(DAY))]], groups=[{0}], aggs=[[]], sort0=[0], dir0=[ASC])";
    sql(sql, FOODMART)
        .explainContains(plan)
  ...
  public void testAggOnArithmeticProject3() {
    ...
    sql(sql, FOODMART)
        ...
        .queryContains(
            druidChecker("'queryType':'groupBy'", "'granularity':'all'",
                "{'dimension':'postagg#0','direction':'ascending','dimensionOrder':'numeric'}",
                "'aggregations':[{'type':'doubleSum','name':'$f1','expression':"
                    + "'((- \\'store_sales\\') * 2)'},{'type':'doubleMax','name':'$f2',"
                    + "'expression':'(\\'store_cost\\' * \\'store_cost\\')'},{'type':'doubleMin',"
                    + "'name':'S2','expression':'(\\'store_sales\\' + \\'store_cost\\')'}],"
                    + "'postAggregations':[{'type':'arithmetic','name':'postagg#0','fn':'-',"
                    + "'fields':[{'type':'fieldAccess','name':'','fieldName':'$f1'},"
                    + "{'type':'fieldAccess','name':'','fieldName':'$f2'}]}],"
            ));
    ...
    sql(sql, FOODMART)
        ...
        .queryContains(
            druidChecker("'queryType':'groupBy'",
                ...
                "{'type':'default','dimension':'vc'}],'virtualColumns':[{'type':'expression','name':'vc"
            ));
  }
