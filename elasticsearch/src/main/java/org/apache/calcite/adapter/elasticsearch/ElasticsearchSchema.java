...
/**
 * Schema mapped onto a index of ELASTICSEARCH types. Each table in the schema
 * is an ELASTICSEARCH type in that index.
 */
public class ElasticsearchSchema extends AbstractSchema {
   ...
   * @param indexName Elasticsearch database name, eg usa.
   */
  public ElasticsearchSchema(Map<String, Integer> coordinates, Map<String, String> userConfig,
                             String indexName) {
    super();
    ...
    if (client != null) {
      String[] indices = client.admin().indices().getIndex(new GetIndexRequest().indices(indexName))
