 ...
 */
public class SplunkDiscoveryDemo {
  
  public static void main(String[] args) throws Exception {
    // Load connection properties
    Properties props = new Properties();
    props.load(SplunkDiscoveryDemo.class.getResourceAsStream("/splunk-connection.properties"));
    
    String url = "jdbc:splunk:" +
        "url='" + props.getProperty("url") + "';" +
        "user='" + props.getProperty("user") + "';" +
        "password='" + props.getProperty("password") + "';" +
        "disableSslValidation='" + props.getProperty("disableSslValidation") + "';" +
        "app='Splunk_SA_CIM';" +
        "datamodelCacheTtl=0";

    System.out.println("Connecting to Splunk with URL: " + url.replaceAll("password='[^']*'", "password='***'"));
