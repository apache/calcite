...
/**
 * Unit test demonstrating SQL to Rel conversion errors on TPC-DS queries 40, 67 and 80.
 */
public class TpcdsTest {
  ...
  public void testQuery(int query) throws IOException, SqlParseException {
  ...
  private List<RelRoot> convertSqlToRel(List<SqlNode> sqlNodes) {
    SqlToRelConverter converter = newSqlToRelConverter();
    return sqlNodes.stream()
        .map(sqlNode -> converter.convertQuery(sqlNode, true, true))
        .collect(Collectors.toList());
  }

  private SqlToRelConverter newSqlToRelConverter() {
    RelOptTable.ViewExpander viewExpander = null;
    SqlValidator validator = new MySqlValidator();
   ...
   */
  private static final class MySqlValidator extends SqlValidatorImpl {
