...
public class IntersectToDistinctRule extends RelOptRule {
  ...
  public IntersectToDistinctRule(Class<? extends Intersect> intersectClazz,
    ...
    this(intersectClazz, RelBuilder.proto(setOpFactory));
  }
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    final int numBranches = intersect.getInputs().size();
    final RexBuilder rexBuilder = intersect.getCluster().getRexBuilder();
    RelDataType aggFnRetType = typeFactory.createSqlType(
            org.apache.calcite.sql.type.SqlTypeName.BIGINT);
    RelBuilder unionBuilder = call.builder();
    ...
    for (int index = 0; index < numBranches; index++) {
      ...
      relBuilder.project(gbChildProjLst);
      RelNode gbInputRel = relBuilder.build();
      ...
      argList.add(input.getRowType().getFieldList().size());
    ...
    unionBuilder.union(true, numBranches);
    ...
    RexLiteral literal = rexBuilder.makeBigintLiteral(new BigDecimal(numBranches));
