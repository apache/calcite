...
public class AggregateReduceFunctionsRule
  ...
  private static final IntPredicate ALWAYS_TRUE = x -> true;
  ...
  private RexNode reduceAgg(
    ...
    if (canReduce(oldCall)) {
      ...
      return rexBuilder.addAggCall(oldCall,
          ...
          newCalls,
          aggCallMapping,
          inputFieldIsNullableUnlessFiltered(oldCall, oldAggRel));
    }
  }
  ...
  /**
   * Return a predicate to determine if an agg call's input is considered nullable, for the sole
   * purpose of pruning non-nullable inputs to {@code COUNT} calls when invoking
   * {@link RexBuilder#addAggCall}.
   *
   * Every filtered agg call generated by this rule should retain the same argument list as the
   * original. A previous version of this rule would discard non-nullable arguments to all generated
   * {@code COUNT} calls, since {@code COUNT(*)} is equivalent to {@code COUNT(x)} for non-nullable
   * {@code x}. This is OK if the rule is used in isolation, however it can produce incorrect
   * results in an edge case when used in conjunction with {@link AggregateExpandWithinDistinctRule}
   * on an aggregate with a filter; e.g. if {@code AVG(a) WITHIN DISTINCT (b) FILTER c} is reduced
   * to {@code SUM(a) WITHIN DISTINCT (b) FILTER (c) / COUNT(*) WITHIN DISTINCT (b) FILTER (c)},
   * and there is a value of {@code b} for which all rows are discarded by the filter {@code c},
   * the "inner agg" produced by the {@code WITHIN DISTINCT} rule will produce a null value for
   * {@code a} in that {@code b}-group, which will be ignored by the numerator but counted by the
   * denominator in the "outer agg".
   */
  private static IntPredicate inputFieldIsNullableUnlessFiltered(AggregateCall oldCall,
      Aggregate oldAggRel) {
    return oldCall.hasFilter() ? ALWAYS_TRUE : oldAggRel.getInput()::fieldIsNullable;
  }
