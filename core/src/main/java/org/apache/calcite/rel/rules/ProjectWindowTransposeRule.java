...
public class ProjectWindowTransposeRule extends RelOptRule {
  ...
  @Override public void onMatch(RelOptRuleCall call) {
    ...
    final RexShuttle indexAdjustment = new RexShuttle() {
      @Override public RexNode visitInputRef(RexInputRef inputRef) {
        final int newIndex = getAdjustedIndex(
            inputRef.getIndex(),
            beReferred,
            windowInputColumn);
    ...
    for (Window.Group group : window.groups) {
      ...
      for (RelFieldCollation relFieldCollation : group.orderKeys.getFieldCollations()) {
        ...
        orderKeys.add(relFieldCollation.copy(getAdjustedIndex(
            index,
            beReferred,
            windowInputColumn)));
      }
  ...
  private int getAdjustedIndex(final int initIndex,
    ...
    if (initIndex >= windowInputColumn) {
      adjustedIndex = beReferred.cardinality() + (initIndex - windowInputColumn);
    } else {
      adjustedIndex = beReferred.get(0, initIndex).cardinality();
    }
