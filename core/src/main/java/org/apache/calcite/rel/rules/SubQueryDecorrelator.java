 ...
 *
 * <p>The Basic idea of SubQueryDecorrelator is from {@link
...
public class SubQueryDecorrelator extends RelShuttleImpl {
   ...
   */
  private static RexNode adjustInputRefs(final RexNode c,
      final Map<Integer, Integer> mapOldToNewIndex, final RelDataType rowType) {
  ...
  /**
   * Shuttle that shifts inputs of correlations.
   */
  private static class DecorrelateRexShuttle extends RexShuttle {
    ...
    private DecorrelateRexShuttle(RelDataType leftRowType, RelDataType rightRowType,
  ...
  public static class SubQueryRelDecorrelator implements ReflectiveVisitor {
      ...
      if (frame.c != null) {
        ...
        for (int inputIndex : corInputIndices.toList()) {
          if (!mapInputToOutput.containsKey(inputIndex)) {
            projects.add(
                newPos,
                Pair.of(RexInputRef.of(inputIndex, inputRowType),
    ...
    public Frame decorrelateRel(LogicalAggregate rel) {
      ...
      relBuilder.push(
          LogicalAggregate.create(newProject, rel.getHints(), newGroupSet, null, newAggCalls));
    ...
    public Frame decorrelateRel(Sort rel) {
      ...
      final RelNode newSort = LogicalSort.create(newInput, newCollation, rel.offset, rel.fetch)
          .withHints(rel.getHints());
  ...
  private static class CorelMapBuilder extends RelShuttleImpl {
    ...
    CorelMapBuilder(int maxCnfNodeCount) {
      this.maxCnfNodeCount = maxCnfNodeCount;
    }
    ...
    final com.google.common.collect.SortedSetMultimap<RelNode, CorRef> mapRefRelToCorRef =
        ...
        .newSortedSetMultimap(new HashMap<RelNode, Collection<CorRef>>(),
   ...
   *
   * <p>For instance, if a RelNode references emp.name multiple times, it would result in multiple
   ...
   * modify the tree.
   *
   * <p>There are three maps:
   *
   * <ol>
   *   <li>{@link #mapRefRelToCorRef} maps a {@link RelNode} to the correlated variables it
   *       references;
   *   <li>{@link #mapCorToCorRel} maps a correlated variable to the {@link RelNode} providing it;
   *   <li>{@link #mapSubQueryNodeToCorSet} maps a {@link RelNode} to the correlated variables it
   *       has;
   * </ol>
   */
  ...
  private static class Frame {
    ...
    Frame(RelNode oldRel, RelNode newRel, RexNode corCondition,
      ...
      assert allLessThan(this.oldToNewOutputs.keySet(), oldRel.getRowType().getFieldCount(),
          Litmus.THROW);
      assert allLessThan(this.oldToNewOutputs.values(), r.getRowType().getFieldCount(),
          Litmus.THROW);
    }
