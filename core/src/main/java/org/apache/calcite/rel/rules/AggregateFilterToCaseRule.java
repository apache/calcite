...
public class AggregateFilterToCaseRule
  ...
  @Override public void onMatch(RelOptRuleCall call) {
    ...
    for (AggregateCall aggregateCall : aggregate.getAggCallList()) {
      AggregateCall newCall = transform(aggregateCall, relBuilder.getRexBuilder(), newProjects);
  ...
  private static AggregateCall transform(AggregateCall call,
      RexBuilder rexBuilder, List<RexNode> newProjects) {
    if (!call.hasFilter()) {
      return call;
    }
    ...
    if (kind == SqlKind.COUNT) {
      ...
      // COUNT() FILTER (x = 'foo') => COUNT(CASE WHEN x = 'foo' THEN 'dummy' END)
      arg1 = call.getArgList().size() == 0
          ? rexBuilder.makeLiteral("dummy")
