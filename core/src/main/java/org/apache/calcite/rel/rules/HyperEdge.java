...
public class HyperEdge {
  ...
  public static List<HyperEdge> createHyperEdgesFromJoinConds(
    ...
    for (RexNode joinCond : joinConds) {
      ...
      if (leftNodeUsedInPredicate == 0 || rightNodeUsedInPredicate == 0) {
        ...
        // set using all nodes on the left and right sides (as the endpoint of the hyperedge). Cross
        // products and degenerate predicates should be rare in real queries, this behavior
        // constrains the search space and make sure a correct plan. See section 6.2 in CD-C paper
  ...
  private static class TotalEligibilitySet {
    ...
    /**
     * For a conflict rule T1 → T2, if T1 ∩ tes != ∅, we can add T2 to tes due to the nature of
     * conflict rule. Further, if T2 ⊆ tes, we can safely eliminate this conflict rule. This
     * behavior has two benefits:
     *
