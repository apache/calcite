...
public interface SubstitutionRule extends TransformationRule {
  ...
  /**
   * Whether the planner should automatically replace references
   * to the old node with the new node in parent inputs when at least one
   * equivalent {@code RelNode} is generated by this rule.
   *
   * <p>Default is false, the user needs to replace the old node
   * manually in the rule.
   */
  default boolean autoReplaceOld() {
    return false;
  }
}
