 ...
 * When removing SOME/IN subqueries, the condition need to be retained in the mark type Correlate
 * (it cannot be pulled up or pushed down). This is why CorrelatePlus extends the condition.
 *
...
public abstract class CorrelatePlus extends Correlate {
  ...
  protected CorrelatePlus(
    ...
    assert joinType == JoinRelType.MARK;
  }

  @Override public CorrelatePlus copy(RelTraitSet traitSet, List<RelNode> inputs) {
    assert inputs.size() == 2;
    return copy(traitSet, inputs.get(0), inputs.get(1), correlationId,
        requiredColumns, joinType, condition);
  }

  public abstract CorrelatePlus copy(RelTraitSet traitSet,
