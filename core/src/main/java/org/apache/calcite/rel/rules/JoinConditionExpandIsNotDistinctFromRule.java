...
public final class JoinConditionExpandIsNotDistinctFromRule
  ...
  private static class RemoveIsNotDistinctFromRexShuttle extends RexShuttle {
    ...
    @Override public RexNode visitCall(RexCall call) {
      ...
      if (call.getOperator()
        ...
        RexNode operand0Zero = makeZeroLiteral(rexBuilder, operand0);
        RexNode operand1Zero = makeZeroLiteral(rexBuilder, operand1);
  ...
  private static RexNode makeZeroLiteral(RexBuilder rexBuilder, RexNode operand) {
    if (operand.getType().getSqlTypeName() == SqlTypeName.ARRAY) {
      ...
      return rexBuilder.makeCall(arrayType,
          SqlStdOperatorTable.ARRAY_VALUE_CONSTRUCTOR,
          ImmutableList.of(elementZero));
    }
    return rexBuilder.makeZeroLiteral(operand.getType());
  }
