...
public class LogicalMatch extends Match {
   ...
   * @param allRows One row or all rows per match
  ...
  public static LogicalMatch create(RelNode input, RexNode pattern,
      ...
      Map<String, RexNode> patternDefinitions, Map<String, RexNode> measures,
      RexNode after, Map<String, TreeSet<String>> subsets, boolean allRows, RelDataType rowType) {
    final RelOptCluster cluster = input.getCluster();
    final RelTraitSet traitSet = cluster.traitSetOf(Convention.NONE);
    return new LogicalMatch(cluster, traitSet, input, pattern,
        strictStart, strictEnd, patternDefinitions, measures, after, subsets, allRows, rowType);
  }
