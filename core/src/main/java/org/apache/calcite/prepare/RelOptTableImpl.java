...
public class RelOptTableImpl extends Prepare.AbstractPreparingTable {
  ...
  @Override public SqlAccessType getAllowedAccess(Principal p) {
    if (schema == null || !(schema instanceof CalciteCatalogReader)) {
      return SqlAccessType.ALL;
    }

    final CalciteCatalogReader catalogReader = (CalciteCatalogReader) schema;
    CalciteSchema calciteSchema =
        SqlValidatorUtil.getSchema(
            catalogReader.getRootSchema(), Util.skipLast(names), catalogReader.nameMatcher());
    if (calciteSchema == null) {
      return SqlAccessType.ALL;
    }
    return calciteSchema.getAccessType(Util.last(names), p, catalogReader.isCaseSensitive());
  }
