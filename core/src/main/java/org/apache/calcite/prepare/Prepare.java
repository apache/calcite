...
public abstract class Prepare {
  ...
  public PreparedResult prepareSql(
    ...
    final SqlToRelConverter.ConfigBuilder builder = SqlToRelConverter.configBuilder()
            .setTrimUnusedFields(true).setExpand(THREAD_EXPAND.get());
    ...
    if (sqlQuery.getKind() == SqlKind.EXPLAIN) {
      ...
      sqlToRelConverter = getSqlToRelConverter(validator, catalogReader,
          builder.setIsExplain(true).build());
      sqlToRelConverter.setDynamicParamCountInExplain(sqlExplain.getDynamicParamCount());
    } else {
      sqlToRelConverter = getSqlToRelConverter(validator, catalogReader, builder.build());
    }
  ...
  protected RelRoot trimUnusedFields(RelRoot root) {
    final SqlToRelConverter.Config config = SqlToRelConverter.configBuilder()
        .setTrimUnusedFields(shouldTrim(root.rel)).setExpand(THREAD_EXPAND.get()).build();
    final SqlToRelConverter converter =
        getSqlToRelConverter(
            getSqlValidator(), catalogReader, config);
