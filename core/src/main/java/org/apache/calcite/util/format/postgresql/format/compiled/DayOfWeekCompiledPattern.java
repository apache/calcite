 ...
 * the week.
 */
public class DayOfWeekCompiledPattern extends CompiledPattern {
  private final CapitalizationEnum capitalization;
  private final TextStyle textStyle;

  public DayOfWeekCompiledPattern(Set<PatternModifier> modifiers,
      CapitalizationEnum capitalization, TextStyle textStyle) {
  ...
  @Override public String convertToString(ZonedDateTime dateTime, Locale locale) {
    ...
    final int intValue = dateTime.getDayOfWeek().getValue();
    final String stringValue =
        capitalization.apply(
            DayOfWeek.of(intValue).getDisplayName(textStyle, localeToUse), localeToUse);
  ...
  @Override public int parseValue(ParsePosition inputPosition, String input, boolean enforceLength,
    ...
    for (DayOfWeek dayOfWeek : DayOfWeek.values()) {
      final String expectedValue =
          capitalization.apply(dayOfWeek.getDisplayName(textStyle, localeToUse), localeToUse);
  ...
  @Override protected int getBaseFormatPatternLength() {
    return textStyle == TextStyle.FULL ? 3 : 2;
  }
}
