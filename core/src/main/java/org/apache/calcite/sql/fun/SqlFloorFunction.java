...
public class SqlFloorFunction extends SqlMonotonicUnaryFunction {
  ...
  private void unparseDatetime(SqlWriter writer, SqlCall call) {
    ...
    SqlDialect.DatabaseProduct dialect = writer.getDialect().getDatabaseProduct();
    if (dialect == SqlDialect.DatabaseProduct.UNKNOWN
        || dialect == SqlDialect.DatabaseProduct.CALCITE) {
      unparseDatetimeDefault(writer, call);
      return;
    }

    SqlLiteral timeUnitNode = call.operand(1);
    TimeUnitRange timeUnit = (TimeUnitRange) timeUnitNode.getValue();
