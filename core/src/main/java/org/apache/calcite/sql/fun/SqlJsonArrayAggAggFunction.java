...
public class SqlJsonArrayAggAggFunction extends SqlAggFunction {
  ...
  @Override protected void preValidateCall(SqlValidator validator, SqlValidatorScope scope,
      SqlCall call) {
    assert nullClause != null;
  }
  ...
  @Override public String toString() {
    return getName() + String.format(Locale.ENGLISH, "<%s>", nullClause);
  }

  public SqlJsonArrayAggAggFunction with(SqlJsonConstructorNullClause nullClause) {
    return new SqlJsonArrayAggAggFunction(getName(), nullClause);
  }
