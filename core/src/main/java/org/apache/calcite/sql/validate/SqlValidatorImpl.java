...
public class SqlValidatorImpl implements SqlValidatorWithHints {
  ...
  /**
   * Exact source-field metadata for identifiers generated while expanding
   * {@code SELECT *}. This preserves positional field identity even when the
   * visible field name is duplicated.
   */
  private final IdentityHashMap<SqlIdentifier, StarExpansionReference>
      starExpansionReferences = new IdentityHashMap<>();
  ...
  /** Exact source-field metadata for an identifier generated by star
   * expansion. */
  static class StarExpansionReference {
    final int prefixLength;
    final SqlValidatorNamespace namespace;
    final SqlIdentifier qualifiedIdentifier;
    final RelDataType type;
    final int fieldIndex;

    StarExpansionReference(int prefixLength, SqlValidatorNamespace namespace,
        SqlIdentifier qualifiedIdentifier, RelDataType type, int fieldIndex) {
      this.prefixLength = prefixLength;
      this.namespace = requireNonNull(namespace, "namespace");
      this.qualifiedIdentifier =
          requireNonNull(qualifiedIdentifier, "qualifiedIdentifier");
  ...
  private boolean expandSelectItem(final SqlNode selectItem, SqlSelect select,
    ...
    // fields.size() is the output ordinal; aliases.size() can be smaller when
  ...
  private boolean expandStar(List<SqlNode> selectItems, Set<String> aliases,
      ...
      for (ScopeChild child : scope.children) {
        ...
        if (child.namespace.getRowType().isDynamicStruct()) {
          ...
          for (RelDataTypeField field : rowType.getFieldList()) {
            ...
            final StarExpansionReference starExpansionReference =
                hasDuplicateFieldName(rowType, field, nameMatcher)
                    ? new StarExpansionReference(1, fromNs, exp,
                        field.getType(), field.getIndex())
                    : null;
            // Don't add expanded rolled up columns
            if (!isRolledUpColumn(exp, scope, starExpansionReference)) {
              addOrExpandField(
                  selectItems,
                  aliases,
                  fields,
    ...
    default:
      ...
      if (rowType.isDynamicStruct()) {
        ...
        final SqlValidatorNamespace prefixNs = resolved.only().namespace;
        for (RelDataTypeField field : rowType.getFieldList()) {
          String columnName = field.getName();
          ...
          final StarExpansionReference starExpansionReference =
              hasDuplicateFieldName(rowType, field, resolvedNameMatcher)
                  ? new StarExpansionReference(prefixId.names.size(), prefixNs,
                      columnId, field.getType(), field.getIndex())
                  : null;
          // TODO: do real implicit collation here
          addOrExpandField(
              selectItems,
  ...
  private static boolean hasDuplicateFieldName(RelDataType rowType,
      RelDataTypeField field, SqlNameMatcher nameMatcher) {
    return nameMatcher.frequency(rowType.getFieldNames(), field.getName()) > 1;
  }
  ...
  private boolean addOrExpandField(List<SqlNode> selectItems, Set<String> aliases,
      ...
      @Nullable StarExpansionReference starExpansionReference) {
  ...
  /**
   * Adds an expression produced by star expansion to a select list.
   */
  private void addToSelectList(
      ...
      @Nullable StarExpansionReference starExpansionReference) {
    // Star expansion should preserve the visible alias exactly, including
    // duplicates across earlier select items or across multiple stars.
    final String finalAlias = SqlValidatorUtil.alias(exp, fieldList.size());
    if (starExpansionReference != null) {
      starExpansionReferences.putIfAbsent(exp, starExpansionReference);
    }
    aliases.add(finalAlias);
    ((PairList<String, RelDataType>) fieldList)
        .add(finalAlias, deriveType(scope, exp));
    list.add(exp);
  }
  ...
  private @Nullable Pair<String, String> findTableColumnPair(SqlIdentifier identifier,
      SqlValidatorScope scope) {
    return findTableColumnPair(identifier, scope, getStarExpansionReference(identifier));
  }
  ...
  private @Nullable Pair<String, String> findTableColumnPair(SqlIdentifier identifier,
    ...
    final List<String> names =
        starExpansionReference != null
            ? starExpansionReference.qualifiedIdentifier.names
            : scope.fullyQualify(identifier).identifier.names;
  ...
  private @Nullable Table resolveTable(SqlIdentifier identifier,
      SqlValidatorScope scope) {
    return resolveTable(identifier, scope, getStarExpansionReference(identifier));
  }
  ...
  private static @Nullable Table resolveTable(SqlIdentifier identifier,
    ...
    final SqlValidatorNamespace namespace;
    if (starExpansionReference != null) {
      namespace = starExpansionReference.namespace;
    } else {
      final SqlQualified fullyQualified = scope.fullyQualify(identifier);
      if (fullyQualified.namespace == null) {
        throw new IllegalArgumentException("namespace must not be null in "
            + fullyQualified);
      }
      namespace = fullyQualified.namespace;
    }
  ...
  private boolean isRolledUpColumn(SqlIdentifier identifier, SqlValidatorScope scope) {
    return isRolledUpColumn(identifier, scope, getStarExpansionReference(identifier));
  }

  private boolean isRolledUpColumn(SqlIdentifier identifier, SqlValidatorScope scope,
      @Nullable StarExpansionReference starExpansionReference) {
    Pair<String, String> pair =
        findTableColumnPair(identifier, scope, starExpansionReference);
    ...
    Table table = resolveTable(identifier, scope, starExpansionReference);
    if (table != null) {
  ...
  @Nullable StarExpansionReference getStarExpansionReference(SqlIdentifier id) {
    return starExpansionReferences.get(id);
  }

  public @Nullable Integer getStarExpansionFieldIndex(SqlIdentifier id) {
    final StarExpansionReference reference = starExpansionReferences.get(id);
    return reference == null ? null : reference.fieldIndex;
  }

  /** Copies star-expansion metadata when an identifier node is cloned during
   * later rewrites. The metadata is keyed by identifier instance, not by text,
   * so a cloned identifier must carry over the original entry explicitly. */
  void copyStarExpansionReference(SqlIdentifier source, SqlIdentifier target) {
    final StarExpansionReference reference = starExpansionReferences.get(source);
    if (reference != null) {
      starExpansionReferences.putIfAbsent(target, reference);
    }
  }
  ...
  private class DeriveTypeVisitor implements SqlVisitor<RelDataType> {
    ...
    @Override public RelDataType visit(SqlIdentifier id) {
      ...
      final StarExpansionReference starExpansionReference =
          getStarExpansionReference(id);
      if (starExpansionReference != null) {
        // Name-based lookup can bind to the wrong duplicate field, so use the
        // exact field type captured during star expansion.
        return starExpansionReference.type;
      }
  ...
  class OrderExpressionExpander extends SqlScopedShuttle {
    ...
    private SqlNode nthSelectItem(int ordinal, final SqlParserPos pos) {
      ...
      // item.
      final SqlNode expr2 = expr.clone(pos);
      if (expr instanceof SqlIdentifier && expr2 instanceof SqlIdentifier) {
        SqlValidatorImpl.this.copyStarExpansionReference((SqlIdentifier) expr,
            (SqlIdentifier) expr2);
      }
      return expr2;
    }
