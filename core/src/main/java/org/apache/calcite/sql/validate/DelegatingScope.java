...
public abstract class DelegatingScope implements SqlValidatorScope {
    ...
    default: {
      ...
      for (Step step : path.steps()) {
        ...
        default:
          ...
          if (hasAmbiguousField(step.rowType, field0, name,
              nameMatcher)) {
            throw validator.newValidationError(identifier,
   ...
   * same named field. Having more than one star columns implies ambiguous column. */
  private boolean hasAmbiguousField(RelDataType rowType,
    ...
    } else { // check for same name field
      int count = 0;
