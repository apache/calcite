...
public abstract class SqlLibraryOperators {
  ...
  /** The "TIME_TRUNC(time_expression, time_part)" function; truncates a TIME value to the
   * granularity of time_part. The TIME value is always rounded to the beginning of time_part. */
  @LibraryOperator(libraries = {BIG_QUERY})
  ...
  /** The "TIMESTAMP_TRUNC(timestamp_expression, date_time_part[, time_zone])" bigquery function;
   * Truncates a TIMESTAMP value to the granularity of date_time_part. The TIMESTAMP value is
   * always rounded to the beginning of date_time_part. */
  @LibraryOperator(libraries = {BIG_QUERY})
