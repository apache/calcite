...
/**
 * Parse tree for {@code REVOKE} statement.
 */
public class SqlRevoke extends SqlAuthCommand {
  private static final SqlOperator OPERATOR =
      new SqlSpecialOperator("REVOKE", SqlKind.REVOKE);

  /** Creates a SqlRevoke. */
  public SqlRevoke(SqlParserPos pos, SqlNodeList accesses,
      SqlNodeList objects, SqlLiteral type, SqlNodeList users) {
    super(OPERATOR, pos, accesses, objects, type, users);
  }

  @Override public SqlKind getKind() {
    return SqlKind.REVOKE;
  }
  ...
  @Override public void unparse(SqlWriter writer, int leftPrec, int rightPrec) {
    ...
    writer.keyword("ON");
    if (type.symbolValue(SqlAuthCommand.ObjectType.class) == ObjectType.SCHEMA) {
