...
public class RelBuilder {
  ...
  private RelBuilder rewriteAggregateWithDuplicateGroupSetsImpl(
    ...
    // If the initial value of groupCount is 0, aggregate function type inference
    // is not mandatory. If the initial value of groupCount is greater than 0, to
    // avoid inconsistent nullable values generated by the aggregate function during
    // the Aggregate expansion process (the new Aggregate may produce empty groups),
    // allowChangeNullable = false is mandatory here.
    if (groupCount > 0) {
