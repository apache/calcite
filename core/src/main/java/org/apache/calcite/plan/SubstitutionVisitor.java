...
public class SubstitutionVisitor {
  ...
  public static @Nullable MutableRel unifyAggregates(MutableAggregate query,
    ...
    if (query.groupSets.equals(target.groupSets)
      ...
      // query's index mapping target's index;
      final Map<Integer, Integer> queryIndexesByTarget = new HashMap<>();
      final List<Integer> queryGroupSet = query.groupSet.asList();
      for (int i = 0; i < queryGroupSet.size(); i++) {
        final Integer inputIndex = queryGroupSet.get(i);
        // Use the index in target group by.
        int indexOfTargetGroup = targetGroupByIndexList.indexOf(inputIndex);
        projects.add(indexOfTargetGroup);
        queryIndexesByTarget.put(i, indexOfTargetGroup);
      }
      final List<AggregateCall> targetGroupGenAggCalls = new ArrayList<>();
      final List<AggregateCall> aggCalls = query.aggCalls;
      for (int i = 0; i < aggCalls.size(); i++) {
        final AggregateCall aggCall = aggCalls.get(i);
        // index of target's agg-calls
        int indexOfTargetAggCalls = target.aggCalls.indexOf(aggCall);
        if (indexOfTargetAggCalls < 0) {
          final AggregateCall newAggCall = genAggCallWithTargetGrouping(
              aggCall, targetGroupByIndexList);
          if (newAggCall == null) {
            return null;
          } else {
          ...
          final int indexOfTarget = groupCount + indexOfTargetAggCalls + projOffset;
          projects.add(indexOfTarget);
          queryIndexesByTarget.put(i + groupCount, indexOfTarget);
        }
      }
      ...
      if (targetGroupGenAggCalls.isEmpty()) {
        ...
        if (target.getGroupType() != Aggregate.Group.SIMPLE
            || query.getGroupType() != Aggregate.Group.SIMPLE) {
          return null;
        }
        // return pattern
        // Calc
        ...
        // Firstly, we create a bottom calc to offer the field,
        // which is used to rollup aggregate and compensate target condition.
        final RexProgramBuilder bottomCalcBuilder = new RexProgramBuilder(
            target.rowType, rexBuilder);
        final List<RelDataTypeField> targetFields = target.rowType.getFieldList();
        final Map<Integer, Integer> targetAggregateToBottomCalcIndexMapping = new HashMap<>();
        for (int i = 0; i < projects.size(); i++) {
          final Integer projIndex = projects.get(i);
          bottomCalcBuilder.addProject(RexInputRef.of(projIndex, targetFields),
              targetFields.get(projIndex).getName());
          targetAggregateToBottomCalcIndexMapping.put(projIndex, i);
        }
        if (targetCond != null) {
          bottomCalcBuilder.addCondition(targetCond);
        }
        final MutableCalc bottomCalc = MutableCalc.of(target, bottomCalcBuilder.getProgram());

        // Secondly, we create a middle rollup aggregate
        // do record elem's index of origin query mapping to elem of middle aggregate.
        final Map<Integer, Integer> originQueryToMidAggregateMapping = new HashMap<>();
        // build grouping of middle aggregate
        final List<Integer> newQueryGroupSet = new ArrayList<>();
        // copy origin index of query's grouping
        for (int i = 0; i < groupCount; i++) {
          originQueryToMidAggregateMapping.put(i, newQueryGroupSet.size());
          newQueryGroupSet.add(i);
        }
        // copy target's agg-calls into grouping
        for (int i = 0; i < aggCalls.size(); i++) {
          final int queryIndex = i + groupCount;
          final Integer indexOfTarget = queryIndexesByTarget.get(queryIndex);
          if (indexOfTarget == null) {
            continue;
          }
          originQueryToMidAggregateMapping.put(queryIndex, newQueryGroupSet.size());
          final Integer bottomCalcIdx = targetAggregateToBottomCalcIndexMapping.get(indexOfTarget);
          assert bottomCalcIdx != null;
          newQueryGroupSet.add(bottomCalcIdx);
        }
        for (int i = 0; i < aggCalls.size(); i++) {
          final int queryIndex = i + groupCount;
          if (originQueryToMidAggregateMapping.containsKey(queryIndex)) {
            continue;
          }
          originQueryToMidAggregateMapping.put(queryIndex, newQueryGroupSet.size() + i);
        }
        // re-build middle aggregate's agg-calls,
        // which args need is from new index of bottom calc.
        final List<AggregateCall> newAggCalls = new ArrayList<>();
        for (AggregateCall aggCall : targetGroupGenAggCalls) {
          final List<Integer> args = aggCall.getArgList();
          final List<Integer> newArgs = new ArrayList<>();
          for (Integer arg : args) {
            final Integer newArg = targetAggregateToBottomCalcIndexMapping.get(arg);
            assert newArg != null;
            newArgs.add(newArg);
          }
          final AggregateCall newAggCall = AggregateCall.create(aggCall.getAggregation(),
              aggCall.isDistinct(), aggCall.isApproximate(),
              aggCall.ignoreNulls(),
              newArgs, -1, aggCall.distinctKeys,
              aggCall.collation, aggCall.type,
              aggCall.name);
          newAggCalls.add(newAggCall);
        }
        final MutableAggregate midAggregate = MutableAggregate.of(bottomCalc,
            ImmutableBitSet.of(newQueryGroupSet), null, newAggCalls);

        // Thirdly, we create a top calc to adjust the order of middle aggregate's fields
        final List<Integer> adjustedProjs = new ArrayList<>();
        for (int i = 0; i < query.rowType.getFieldList().size(); i++) {
          final Integer indexOfMidAggregate = originQueryToMidAggregateMapping.get(i);
          assert indexOfMidAggregate != null && indexOfMidAggregate >= 0;
          adjustedProjs.add(indexOfMidAggregate);
        }
        final List<RexNode> topProjs = MutableRels.createProjectExprs(midAggregate, adjustedProjs);
        final RexProgram topRexProgram = RexProgram.create(
            midAggregate.rowType, topProjs, null, query.rowType, rexBuilder);
        final MutableCalc topCalc = MutableCalc.of(midAggregate, topRexProgram);
        result = topCalc;
      }
