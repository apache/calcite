...
public class ExpandJoinOnRightCalcToJoinUnifyRule
  ...
  public static final ExpandJoinOnRightCalcToJoinUnifyRule INSTANCE =
      new ExpandJoinOnRightCalcToJoinUnifyRule();

  private ExpandJoinOnRightCalcToJoinUnifyRule() {
    super(
  ...
  protected SubstitutionVisitor.@Nullable UnifyResult apply(SubstitutionVisitor.UnifyRuleCall call) {
    ...
    if (!isSupportedPredict(target.condition, joinConditionColumnPairs, fieldCnt(qInput0))) {
      return null;
    }
    ...
    if (splitted != null && splitted.isAlwaysTrue()) {
      ...
      for (int i = 0; i < query.rowType.getFieldCount(); i++) {
        ...
        if (i < fieldCnt(qInput0)) {
          rexInputRef = new RexInputRef(i, query.rowType.getFieldList().get(i).getType());
        } else {
          rexInputRef = RexUtil.shift(qInput1Projs.get(i - fieldCnt(qInput0)),
      ...
      final RexNode compenCond =
          addJoinDerivePredicate(rexBuilder, joinContext, RexUtil.shift(qInput1Cond,
              qInput0.rowType.getFieldCount()));
