...
public class LexCaseSensitiveTest {
  ...
  private static Planner getPlanner(
      List<RelTraitDef> traitDefs, SqlParser.Config parserConfig, Program... programs) {
  ...
  private static void runProjectQueryWithLex(Lex lex, String sql)
    ...
    if (lex.caseSensitive) {
      assertTrue("EMPID".equals(fieldNames.get(0)) && "empid".equals(fieldNames.get(1)));
    } else {
      assertTrue(("EMPID".equals(fieldNames.get(0)) && "empid0".equals(fieldNames.get(1)))
          || "EMPID0".equals(fieldNames.get(0)) && "empid".equals(fieldNames.get(1)));
    }
  }
  ...
  @Test public void testCalciteCaseMySql()
      throws SqlParseException, ValidationException, RelConversionException {
    String sql =
        "select empid as EMPID, empid from (select empid from emps order by `EMPS`.DEPTNO)";
