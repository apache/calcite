...
public class DateRangeRulesTest {
  ...
  @Test public void testExtractYearFromDateColumn() {
    ...
    final RexNode e = f.eq(f.literal(2014), f.exYearDt);
    ...
    checkDateRange(f, f.eq(f.exYearDt, f.literal(2014)),
        is("AND(>=($8, 2014-01-01), <($8, 2015-01-01))"));
    checkDateRange(f, f.ge(f.exYearDt, f.literal(2014)),
        is(">=($8, 2014-01-01)"));
    checkDateRange(f, f.gt(f.exYearDt, f.literal(2014)),
        is(">=($8, 2015-01-01)"));
    checkDateRange(f, f.lt(f.exYearDt, f.literal(2014)),
        is("<($8, 2014-01-01)"));
    checkDateRange(f, f.le(f.exYearDt, f.literal(2014)),
        is("<($8, 2015-01-01)"));
    checkDateRange(f, f.ne(f.exYearDt, f.literal(2014)),
  ...
  @Test public void testExtractYearAndMonthFromDateColumn() {
    final Fixture2 f = new Fixture2();
    checkDateRange(f,
        f.and(f.eq(f.exYearDt, f.literal(2014)), f.eq(f.exMonthDt, f.literal(6))),
  ...
  @Test public void testExtractYearAndMonthFromDateColumn2() {
    ...
    final String s2 = "AND(>=($8, 2000-01-01), <($8, 2001-01-01),"
        ...
        + "AND(>=($8, 2000-05-01), <($8, 2000-06-01))))";
    final RexNode e =
        f.and(f.eq(f.exYearDt, f.literal(2000)),
            f.or(f.eq(f.exMonthDt, f.literal(2)),
                f.eq(f.exMonthDt, f.literal(3)),
                f.eq(f.exMonthDt, f.literal(5))));
  ...
  @Test public void testExtractYearAndDayFromDateColumn() {
    final Fixture2 f = new Fixture2();
    checkDateRange(f,
        f.and(f.eq(f.exYearDt, f.literal(2010)), f.eq(f.exDayDt, f.literal(31))),
  ...
  @Test public void testExtractYearMonthDayFromDateColumn() {
    ...
    // folding intervals.
    checkDateRange(f,
        f.and(f.gt(f.exYearDt, f.literal(2010)), f.lt(f.exYearDt, f.literal(2020)),
            f.eq(f.exMonthDt, f.literal(2)), f.eq(f.exDayDt, f.literal(29))),
        is("AND(>=($8, 2011-01-01),"
  ...
  @Test public void testExtractYearMonthDayFromTimestampColumn() {
    final Fixture2 f = new Fixture2();
    checkDateRange(f,
        f.and(f.gt(f.exYearDt, f.literal(2010)),
            f.lt(f.exYearDt, f.literal(2020)),
            f.eq(f.exMonthDt, f.literal(2)), f.eq(f.exDayDt, f.literal(29))),
        is("AND(>=($8, 2011-01-01),"
            + " AND(>=($8, 2011-01-01), <($8, 2020-01-01)),"
            + " OR(AND(>=($8, 2011-02-01), <($8, 2011-03-01)),"
        ...
        f.or(
            f.and(f.eq(f.exYearDt, f.literal(2000)),
                f.or(f.eq(f.exMonthDt, f.literal(2)),
                    f.eq(f.exMonthDt, f.literal(3)),
                    f.eq(f.exMonthDt, f.literal(5)))),
            f.and(f.eq(f.exYearDt, f.literal(2001)),
                f.eq(f.exMonthDt, f.literal(1)))),
  ...
  @Test public void testExtractWithOrCondition2() {
    ...
    checkDateRange(f,
        f.and(
            f.or(f.eq(f.exYearDt, f.literal(2000)),
                f.eq(f.exYearDt, f.literal(2001))),
            f.or(
                f.and(f.eq(f.exYearDt, f.literal(2000)),
                    f.or(f.eq(f.exMonthDt, f.literal(2)),
                        f.eq(f.exMonthDt, f.literal(3)),
  ...
  @Test public void testExtractPartialRewriteForNotEqualsYear() {
    ...
    checkDateRange(f,
        f.and(
            f.ne(f.exYearDt, f.literal(2000)),
            f.or(
                f.and(f.eq(f.exYearDt, f.literal(2000)),
                    f.or(f.eq(f.exMonthDt, f.literal(2)),
                        f.eq(f.exMonthDt, f.literal(3)),
  ...
  @Test public void testExtractPartialRewriteForInMonth() {
    ...
    checkDateRange(f,
        f.and(
            f.or(f.eq(f.exMonthDt, f.literal(1)),
                f.eq(f.exMonthDt, f.literal(2)),
                f.eq(f.exMonthDt, f.literal(3)),
                f.eq(f.exMonthDt, f.literal(4)),
                f.eq(f.exMonthDt, f.literal(5))),
            f.or(
                f.and(f.eq(f.exYearDt, f.literal(2000)),
                    f.or(f.eq(f.exMonthDt, f.literal(2)),
  ...
  private static class Fixture2 extends Fixture {
    ...
    private final RexNode exYearDt;
    private final RexNode exMonthDt;
    private final RexNode exDayDt;
    ...
    Fixture2() {
      ...
      exYearDt = rexBuilder.makeCall(SqlStdOperatorTable.EXTRACT,
          ImmutableList.of(rexBuilder.makeFlag(TimeUnitRange.YEAR), dt));
      exMonthDt = rexBuilder.makeCall(intRelDataType,
          SqlStdOperatorTable.EXTRACT,
          ImmutableList.of(rexBuilder.makeFlag(TimeUnitRange.MONTH), dt));
      exDayDt = rexBuilder.makeCall(intRelDataType,
          SqlStdOperatorTable.EXTRACT,
