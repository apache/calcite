...
public class DateRangeRulesTest {
  ...
  @Test public void testExtractYearFromDateColumn() {
    ...
    assertThat(DateRangeRules.extractTimeUnits(e),
        is(list(TimeUnitRange.YEAR)));
    assertThat(DateRangeRules.extractTimeUnits(f.dec), is(list()));
    assertThat(DateRangeRules.extractTimeUnits(f.literal(1)), is(list()));
  ...
  @Test public void testExtractYearMonthDayFromTimestampColumn() {
   ...
   // Test case 1 for https://issues.apache.org/jira/browse/CALCITE-1658
  ...
  // Test case 2 for https://issues.apache.org/jira/browse/CALCITE-1658
  @Test public void testExtractWithOrCondition2() {

    /**
     * EXTRACT(YEAR FROM __time) IN (2000, 2001) AND
     * ((EXTRACT(YEAR FROM __time) = 2000 AND EXTRACT(MONTH FROM __time) IN (2, 3, 5))
     * OR (EXTRACT(YEAR FROM __time) = 2001 AND EXTRACT(MONTH FROM __time) = 1))
     */
    final Fixture2 f = new Fixture2();
  ...
  // Test case 3 for https://issues.apache.org/jira/browse/CALCITE-1658
  ...
  @Test public void testExtractPartialRewriteForInMonth() {
    ...
    /**
     * EXTRACT(MONTH FROM __time) in (1,2,3,4,5) AND
     * ((EXTRACT(YEAR FROM __time) = 2000 AND EXTRACT(MONTH FROM __time) IN (2, 3, 5))
     * OR (EXTRACT(YEAR FROM __time) = 2001 AND EXTRACT(MONTH FROM __time) = 1))
     */
    final Fixture2 f = new Fixture2();
  ...
  private static List<TimeUnitRange> list(TimeUnitRange... es) {
