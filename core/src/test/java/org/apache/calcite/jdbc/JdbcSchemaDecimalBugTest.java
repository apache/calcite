 ...
 *
 * <p>These are integration tests that require live database instances.
 * Run with:
 * <pre>{@code ./gradlew :core:integTestCalcite6654}</pre>
 *
 * <p>Required Docker containers – start with:
 * <pre>{@code
 * docker compose -f core/src/test/resources/docker-compose.yml up -d
 * }</pre>
 *
 * <p>Tests are skipped automatically if a container is unreachable.
 */

@Tag("integration")
@EnabledIfSystemProperty(named = "calcite.integration.tests", matches = "true")
class JdbcSchemaDecimalBugTest {
  ...
  /**
   * Executes DDL and DML statements to set up a test table.
   * Silently ignores the drop statement if the table does not exist.
   */
  ...
  @Test void oracleNumberWithoutPrecisionShouldNotThrow() throws Exception {
    ...
    createTestTable(ds,
        "BEGIN EXECUTE IMMEDIATE 'DROP TABLE TEST_NUMBERS';"
