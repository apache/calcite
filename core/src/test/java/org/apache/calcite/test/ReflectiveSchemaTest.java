  ...
  @Test public void testViewPath() throws SQLException, ClassNotFoundException {
    ...
    schema.add("emps",
        ViewTable.viewMacro(schema,
            ...
            Collections.singletonList("hr"), Arrays.asList("s", "emps"), null));
    schema.add("hr_emps",
        ViewTable.viewMacro(schema,
            "select * from \"emps\"",
            Collections.singletonList("hr"), Arrays.asList("s", "hr_emps"), null));
    schema.add("s_emps",
        ViewTable.viewMacro(schema,
            "select * from \"emps\"",
            Collections.singletonList("s"), Arrays.asList("s", "s_emps"), null));
    schema.add("null_emps",
        ViewTable.viewMacro(schema, "select * from \"emps\"", null,
            Arrays.asList("s", "null_emps"), null));
    rootSchema.add("hr", new ReflectiveSchema(new JdbcTest.HrSchema()));
