...
public class EnumerableCorrelateTest {
  ...
  @Test public void leftOuterJoinCorrelate() {
    tester(false, new JdbcTest.HrSchema())
        ...
        .explainContains(""
            + "EnumerableCalc(expr#0..4=[{inputs}], empid=[_t0], name=[_t2], dept=[_t4])\n"
            + "  EnumerableCorrelate(correlation=[_cor0], joinType=[left], requiredColumns=[{1}])\n"
            + "    EnumerableCalc(expr#0..4=[{inputs}], proj#0..2=[{exprs}])\n"
            + "      EnumerableTableScan(table=[[s, emps]])\n"
            + "    EnumerableCalc(expr#0..3=[{inputs}], expr#4=[_cor0], expr#5=[_t4.deptno], expr#6=[=(_t5, _t0)], proj#0..1=[{exprs}], _condition=[_t6])\n"
  ...
  @Test public void simpleCorrelateDecorrelated() {
    tester(true, new JdbcTest.HrSchema())
        ...
        .explainContains(""
            + "EnumerableCalc(expr#0..2=[{inputs}], empid=[_t0], name=[_t2])\n"
            + "  EnumerableHashJoin(condition=[=(_1, _3)], joinType=[semi])\n"
            + "    EnumerableCalc(expr#0..4=[{inputs}], proj#0..2=[{exprs}])\n"
            + "      EnumerableTableScan(table=[[s, emps]])\n"
            + "    EnumerableTableScan(table=[[s, depts]])")
        .returnsUnordered(
  ...
  @Test public void semiJoinCorrelate() {
    tester(false, new JdbcTest.HrSchema())
        ...
        .explainContains(""
            + "EnumerableCalc(expr#0..3=[{inputs}], empid=[_t1], name=[_t3])\n"
            + "  EnumerableCorrelate(correlation=[_cor2], joinType=[inner], requiredColumns=[{0}])\n"
            + "    EnumerableAggregate(group=[{0}])\n"
            + "      EnumerableTableScan(table=[[s, depts]])\n"
            + "    EnumerableCalc(expr#0..4=[{inputs}], expr#5=[_cor2], expr#6=[_t5.deptno], expr#7=[=(_t1, _t6)], proj#0..2=[{exprs}], _condition=[_t7])\n"
            + "      EnumerableTableScan(table=[[s, emps]])")
        .returnsUnordered(
            "empid=100; name=Bill",
            "empid=110; name=Theodore",
  ...
  @Test public void semiJoinCorrelateWithFilterCorrelateRule() {
    tester(false, new JdbcTest.HrSchema())
        ...
        .explainContains(""
            + "EnumerableCalc(expr#0..3=[{inputs}], empid=[_t1], name=[_t3])\n"
            + "  EnumerableCorrelate(correlation=[_cor5], joinType=[inner], requiredColumns=[{0}])\n"
            + "    EnumerableAggregate(group=[{0}])\n"
            + "      EnumerableTableScan(table=[[s, depts]])\n"
            + "    EnumerableCalc(expr#0..4=[{inputs}], expr#5=[100], expr#6=[>(_t0, _t5)], expr#7=[_cor5], expr#8=[_t7.deptno], expr#9=[=(_t1, _t8)], expr#10=[AND(_t6, _t9)], proj#0..2=[{exprs}], _condition=[_t10])\n"
            + "      EnumerableTableScan(table=[[s, emps]])")
        .returnsUnordered(
            "empid=110; name=Theodore",
            "empid=150; name=Sebastian");
  ...
  @Test public void simpleCorrelate() {
    tester(false, new JdbcTest.HrSchema())
        .query(
            "select empid, name from emps e where exists (select 1 from depts d where d.deptno=e.deptno)")
        .explainContains(""
            + "EnumerableCalc(expr#0..3=[{inputs}], empid=[_t0], name=[_t2])\n"
            + "  EnumerableCorrelate(correlation=[_cor0], joinType=[inner], requiredColumns=[{1}])\n"
            + "    EnumerableCalc(expr#0..4=[{inputs}], proj#0..2=[{exprs}])\n"
            + "      EnumerableTableScan(table=[[s, emps]])\n"
            + "    EnumerableAggregate(group=[{0}])\n"
            + "      EnumerableCalc(expr#0..3=[{inputs}], expr#4=[true], expr#5=[_cor0], expr#6=[_t5.deptno], expr#7=[=(_t0, _t6)], i=[_t4], _condition=[_t7])\n"
            + "        EnumerableTableScan(table=[[s, depts]])")
        .returnsUnordered(
            "empid=100; name=Bill",
            "empid=110; name=Theodore",
