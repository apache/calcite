...
public class RexProgramTest {
  ...
  @Test public void testThresholdCnf() {
    ...
    // transformation creates 7 nodes AND(OR(=(?0.x, 1), =(?0.x, 2)), OR(=(?0.x, 1), =(?0.y, 3)))
    // thus, it is triggered
    checkThresholdCnf(or(eq(xRef, literal1), and(eq(xRef, literal2), eq(yRef, literal3))),
        8, "AND(OR(=(?0.x, 1), =(?0.x, 2)), OR(=(?0.x, 1), =(?0.y, 3)))");

    // OR(=(?0.x, 1), =(?0.x, 2), AND(=(?0.x, 3), =(?0.y, 4)))
    // transformation creates 9 nodes AND(OR(=(?0.x, 1), =(?0.x, 2), =(?0.x, 3)),
