...
public class MockSecDataGenerator {
      ...
      for (String filingType : filingTypes) {
        for (int year = startYear; year <= endYear; year++) {
          ...
          for (int quarter = 1; quarter <= filingCount; quarter++) {
            ...
            File partitionDir = new File(baseDirectory,
                String.format("cik=%s/filing_type=%s/year=%d", 
                    cik, filingType.replace("-", ""), year));
            partitionDir.mkdirs();
            
            // Create Parquet file
            String fileName = String.format("%s_%s_%d_Q%d.parquet", 
                cik, filingType.replace("-", ""), year, quarter);
            File parquetFile = new File(partitionDir, fileName);
            
            // Write data
            List<GenericRecord> records = createMockRecords(
                schema, cik, companyName, filingType, year, quarter);
  ...
  private List<GenericRecord> createMockRecords(Schema schema, String cik, 
    ...
    record.put("accession", String.format("%s-%d-%06d", 
